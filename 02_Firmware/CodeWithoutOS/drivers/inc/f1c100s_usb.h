...
#define USBC_MAX_EP_NUM 3 // ep0~2, ep的个数
...
void USBC_INT_ClearEpPendingAll(uint32_t ep_type);
/* clear all the usb misc's interrupt flag */
void USBC_INT_ClearMiscPendingAll();
void USBC_Dev_ConectSwitch(uint32_t is_on);

void USBC_INT_DisableUsbMiscAll(void);
void USBC_INT_DisableEpAll(uint32_t ep_type);

void USBC_Dev_ConfigTransferMode(uint8_t ts_type, uint8_t speed_mode);
void USBC_INT_EnableUsbMiscUint(uint32_t mask);
void USBC_INT_EnableEp(uint32_t ep_type, uint8_t ep_index);

uint32_t USBC_INT_MiscPending(void);
uint32_t USBC_INT_EpPending(uint32_t ep_type);
void USBC_INT_ClearMiscPending(uint32_t mask);
void USBC_INT_ClearEpPending(uint32_t ep_type, uint8_t ep_index);
uint32_t USBC_Dev_QueryTransferMode(void);
uint32_t USBC_GetActiveEp(void);
void USBC_Dev_SetAddress_default(void);
void USBC_SelectActiveEp(uint8_t ep_index);
int USBC_Dev_IsEpStall(uint32_t ep_type);
int USBC_Dev_EpSendStall(uint32_t ep_type);
uint32_t USBC_ReadPacket(void *fifo, uint32_t cnt, void *buff);
uint32_t USBC_WritePacket(void *fifo, uint32_t cnt, void *buff);
void *USBC_SelectFIFO(uint32_t ep_index);
int USBC_Dev_WriteDataStatus(uint32_t ep_type, uint32_t complete);
int USBC_Dev_ReadDataStatus(uint32_t ep_type, uint32_t complete);
void USBC_Dev_Ctrl_ClearSetupEnd(void);
void USBC_Dev_SetAddress(uint8_t address);
int USBC_Dev_EpClearStall(uint32_t ep_type);
int USBC_Dev_Ctrl_IsSetupEnd(void);
int USBC_Dev_Ctrl_IsSetupEnd(void);
uint32_t USBC_Dev_IsReadDataReady(uint32_t ep_type);
uint32_t USBC_ReadLenFromFifo(uint32_t ep_type);
int USBC_Dev_ConfigEp(uint32_t ts_type, uint32_t ep_type, uint32_t is_double_fifo, uint32_t ep_MaxPkt);
void USBC_Clear_Feature_HALT(uint32_t ep_type);
