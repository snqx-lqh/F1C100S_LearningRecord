...
static void sys_uart_init(void)
{
    ...
    clk_enable(CCU_BUS_CLK_GATE2, 21);                                  // Open the clock gate for uart0
    clk_reset_clear(CCU_BUS_SOFT_RST2, 21);                             // Deassert uart0 reset
    uart_init(UART1, 115200);                                           // Configure UART0 to 115200-8-n-1
}
...
void delay_soft(uint32_t delay)
{
    uint32_t max=delay*600;
